$(document).ready(function() {
  ...
  function GetDomainInfo() {
    ...
    let contract = web3.eth.contract(ddnsContractABI).at(ddnsContractAddress);
    contract.GetDomainInfo($("#textareaDomainName").val(), function(
      err,
      result
    ) {
      ...
      if (domainLockTime > 0) $("#textareaDomainValidityTime").val(displayDate);
      else
        $("#textareaDomainValidityTime").val(
      ...
      if (ipfsHash !== "Not Available" && ipfsHash !== "n/a") {
        ...
        let html = jQuery("<div>");
        let div = jQuery("<div>");
        div.append(jQuery(`<img src="${url}" class="ipfsImage"/>`));
        html.append(div);
        html.append("</div>");
  ...
  function RegisterDomain() {
    ...
    contract.RegisterDomain(
      $("#textareaDomainNameRegister").val(),
      $("#textareaDomainIP").val(),
      { value: $("#textareaETH").val() * 1000000000000000000 },
      function(err, txHash) {
        if (err) return showError("Smart contract call failed: " + err);
        else showInfo(`Transaction hash: ${txHash}`);
      }
    );
  }
  ...
  function UploadDomainDocument() {
    ...
    fileReader.onload = function() {
      ...
      let contract = web3js.eth
        .contract(ddnsContractABI)
        .at(ddnsContractAddress);
      ipfs.files.add(fileBuffer, (err, result) => {
        if (err) return showError(err);
        if (result) {
          let ipfsHash = result[0].hash;
          contract.AddDomainInfoDocument(
            $("#textareaDomainNameUpload").val(),
            ipfsHash,
            function(err, txHash) {
              if (err) return showError("Smart contract call failed: " + err);
              showInfo(
                `IPFS Document ${ipfsHash}. Transaction hash: ${txHash}`
              );
            }
          );
        }
  ...
  function ContractOwnerWithdraw() {
    ...
    contract.ContractOwnerWithdraw(
      $("#textareaAmount").val() * 1000000000000000000,
      function(err, txHash) {
        if (err) return showError("Smart contract call failed: " + err);

        showInfo(
          `Amount <b>successfully transferred</b>. Transaction hash: ${txHash}`
        );
      }
    );
  }
